"""
IoUComplete Training Script - Enhanced Version
=============================================
Ultra-optimized training with:
1. MixUp augmentation
2. Label smoothing
3. Pre-trained weight initialization
4. Longer training
5. Enhanced learning rate scheduling

This version aims to surpass IoU Optimized!
"""
...
def train_model(
    ...
    project_name='IoUComplete_Enhanced',
    pretrained_path=None,
    mixup_alpha=0.2,
    use_mixup=True
):
    """Train the IoUComplete model with enhanced features"""
    ...
    if pretrained_path and Path(pretrained_path).exists():
        print(f"\n📥 Loading pre-trained encoder weights from: {pretrained_path}")
        ...
        if 'model_state_dict' in pretrained:
            ...
            start_epoch = pretrained.get('epoch', 0) + 1
        else:
            pretrained_state = pretrained  # Direct state dict
            start_epoch = 0
        
        # Load ONLY compatible encoder weights (backbone)
        model_dict = model.state_dict()
        pretrained_encoder = {}
        loaded_count = 0
        
        for key in pretrained_state:
            if key.startswith('backbone.') and key in model_dict:
                if pretrained_state[key].shape == model_dict[key].shape:
                    pretrained_encoder[key] = pretrained_state[key]
                    loaded_count += 1
        
        model_dict.update(pretrained_encoder)
        model.load_state_dict(model_dict)
        
        print(f"✅ Loaded {loaded_count} compatible encoder weights from pre-trained model")
        print(f"   Transformer and decoder weights initialized from scratch")
        print(f"   Starting fresh training (epoch 0)")
    ...
    history = {
        ...
        'val_mse': [], 'val_pearson': [],
        'learning_rates': [],
        'loss_components': {'bce': [], 'dice': [], 'iou': [], 'lovasz': [], 'boundary': []}
    }
    
    best_val_iou = 0.0
    patience_counter = 0
    patience = 80  # Longer patience for enhanced training
    
    print(f"\n{'='*60}")
    print("Starting IoUComplete Enhanced Training")
    print(f"{'='*60}")
    print(f"Device: {device}")
    print(f"Epochs: {num_epochs} (starting from {start_epoch})")
    print(f"Training samples: {len(train_loader.dataset)}")
    print(f"Validation samples: {len(val_loader.dataset)}")
    print(f"Use MixUp: {use_mixup} (alpha={mixup_alpha})")
    print(f"{'='*60}\n")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        model.train()
        train_loss = 0.0
        loss_components_sum = {'bce': 0.0, 'dice': 0.0, 'iou': 0.0, 'lovasz': 0.0, 'boundary': 0.0}
        ...
        for batch_idx, (images, labels) in enumerate(pbar):
            ...
            if use_mixup and np.random.rand() > 0.3:  # 70% of the time
                images, y_a, y_b, lam = mixup_data(images, labels, alpha=mixup_alpha)
                
                # Create combined targets
                target_a_center = y_a
                target_a_distance = y_a.expand(-1, 32, -1, -1)
                target_a = torch.cat([target_a_center, target_a_distance], dim=1)
                
                target_b_center = y_b
                target_b_distance = y_b.expand(-1, 32, -1, -1)
                target_b = torch.cat([target_b_center, target_b_distance], dim=1)
            else:
                lam = 1.0
                target_center = labels
                target_distance = labels.expand(-1, 32, -1, -1)
                target = torch.cat([target_center, target_distance], dim=1)
            ...
            if use_mixup and lam < 1.0:
                # MixUp loss
                loss_a, components_a = criterion(pred, target_a)
                loss_b, components_b = criterion(pred, target_b)
                loss = lam * loss_a + (1 - lam) * loss_b
                
                # Average components
                components = {}
                for k in components_a.keys():
                    components[k] = lam * components_a[k] + (1 - lam) * components_b[k]
            else:
        ...
        if val_iou > best_val_iou:
            ...
            torch.save(checkpoint, save_dir / 'ioucomplete_enhanced_best.pth')
            print(f"  ✅ New best model saved! IoU: {val_iou:.4f}")
            
            # Print comparison with IoU Optimized
            print(f"\n  📊 vs IoU Optimized:")
            print(f"     IoU: {val_iou:.4f} vs 0.9736 = {val_iou - 0.9736:+.4f}")
            print(f"     Dice: {val_dice:.4f} vs 0.9866 = {val_dice - 0.9866:+.4f}")
        else:
            patience_counter += 1
    ...
    if best_val_iou >= 0.9736:
        print("\n🎉 CONGRATULATIONS! IoUComplete surpassed IoU Optimized! 🎉")
    else:
        print(f"\n⚠️ Best IoU: {best_val_iou:.4f} (Target: 0.9736)")
        print(f"   Gap: {0.9736 - best_val_iou:.4f}")
    
    # Save history
    with open(save_dir / 'training_history_enhanced.json', 'w') as f:
...
def save_training_curves(history, save_dir):
    ...
    axes[0, 1].axhline(y=0.9866, color='r', linestyle='--', label='IoU Optimized')
    ...
    axes[0, 2].axhline(y=0.9736, color='r', linestyle='--', label='IoU Optimized')
    ...
    axes[1, 0].axhline(y=0.0175, color='r', linestyle='--', label='IoU Optimized')
    ...
    axes[1, 1].axhline(y=0.9844, color='r', linestyle='--', label='IoU Optimized')
    ...
    plt.tight_layout()
    plt.savefig(save_dir / 'ioucomplete_enhanced_training_curves.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    ax.grid(True)
    plt.tight_layout()
    plt.savefig(save_dir / 'ioucomplete_enhanced_loss_components.png', dpi=150, bbox_inches='tight')
    plt.close()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train IoUComplete Enhanced Model')
    ...
    parser.add_argument('--pretrained', type=str, default='models_complete/complete_much_better_best.pth',
    ...
    # Initialize loss function with BoundaryLoss
    criterion = CombinedIoUCompleteLoss(
        bce_weight=0.15,
        dice_weight=0.25,
        iou_weight=0.25,
        lovasz_weight=0.20,
        boundary_weight=0.15,  # Add boundary loss!
        label_smoothing=args.label_smoothing
    )
