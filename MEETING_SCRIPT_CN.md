...
**最佳模型：hdrg9_finetuned（完整三步骤架构）**

| 指标 | 数值 | 备注 |
|------|------|------|
| **IoU** | **81.8%** | 超过80%目标阈值 |
| **Dice Score** | 89.3% | 接近90%优秀水平 |
| **稳定性** | 标准差降低31.3% | 相比baseline显著提升 |

**模型配置**：
- **架构**：完整三步骤（FPN + Transformer Shape Prior + Deformable Convolution）
- **Base Channels**：48（大容量模型）
- **Shape Prototypes**：8个可学习原型
- **参数量**：约7.5M
...
- 最佳评估配置：TTA Only（测试时增强）

**稳定性分析**：
- 标准差从0.198降低到0.136（降低31.3%）
- 相比baseline，稳定性显著提升
- 证明了模型的鲁棒性和泛化能力"
...
最终模型（81.8% IoU）使用了完整的三步骤架构（FPN + Transformer Shape Prior + Deformable Convolution），所有组件都启用，实现了最佳性能。"
...
**Q3: 最终模型使用了哪些组件？为什么选择完整的三步骤架构？**

"A: 最终达到81.8% IoU的模型使用了完整的三步骤架构：
- **FPN**：提供多尺度特征融合，贡献约+5.2% IoU
- **Transformer Shape Prior**：学习形状先验，贡献约+2.1% IoU
- **Deformable Convolution**：自适应采样，贡献约+3.8% IoU

通过ablation studies发现，虽然每个组件单独都有贡献，但三个组件协同工作时效果最佳。完整架构的81.8% IoU显著高于单独使用FPN的77.92%，证明了三个创新点的协同效应。"
